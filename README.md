...
│   │   └── ticketService.ts         # Ticket 도메인 서비스 (확장 포인트)
...
│   │   │   └── auth.rs              # Auth 도메인 Commands (verify_credentials 등)
│   │   ├── services/                # 하드웨어 제어 및 비즈니스 로직
│   │   │   └── http.rs              # 백엔드 통신용 HTTP 클라이언트 (reqwest)
│   │   ├── utils/
│   │   │   └── error.rs             # reqwest 에러 → String 변환 공통 유틸
...
│   └── tauri.conf.json              # Tauri 앱 설정 (윈도우, 번들, 빌드 등)
...
> | HTTP 클라이언트 | `src/main/api/axiosInstance.ts` (Axios) | `src-tauri/src/services/http.rs` (reqwest) |
> | 에러 변환 유틸 | `src/main/api/utils/ipcErrorHandler.ts` | `src-tauri/src/utils/error.rs` |
...
| **Types** | `src/types/ipc.ts` | **[공통 타입]** `CommandResponse<T>` – 모든 Command 응답의 공통 인터페이스 |
| **Command Handler** | `src-tauri/src/commands/` | **[도메인 로직]** `#[tauri::command]` 함수 (파일 1개 = 도메인 1개) |
| **Error Util** | `src-tauri/src/utils/error.rs` | **[공통 에러 변환]** reqwest 에러 → Rust `String` 로 일관된 변환 |
| **HTTP Client** | `src-tauri/src/services/http.rs` | **[HTTP 클라이언트]** 백엔드 통신 + 인증 토큰 관리 (`reqwest`) |
| **Main (Rust)** | `src-tauri/src/main.rs` | **[진입점]** 모든 Command를 한 곳에서 등록하는 오케스트레이터 |
...
use commands::ticket::get_sale_list; // ← 추가
